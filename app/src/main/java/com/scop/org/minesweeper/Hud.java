...
public class Hud {
	...
	public void draw(Canvas canvas) {
		...
		canvas.drawRect(0, ScreenProperties.HEIGHT-margin*2-fontHeight,
				ScreenProperties.WIDTH, ScreenProperties.HEIGHT, bg);

		textStyle.setTextAlign(Paint.Align.LEFT);
		canvas.drawText("Open: "+(logic.getRevisedTiles())+"/"+logic.getTotalTiles(),
				margin, ScreenProperties.HEIGHT-margin, textStyle);
		...
		canvas.drawText(rightText, ScreenProperties.WIDTH-margin,
				ScreenProperties.HEIGHT-margin, textStyle);
	}
	...
	public void timerNotify(int time){
		...
		if (showTime) {
			view.postInvalidate(0, (int) (ScreenProperties.HEIGHT-margin-fontHeight),
					ScreenProperties.WIDTH, ScreenProperties.HEIGHT);
		}
	}
